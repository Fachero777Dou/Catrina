...
public class TransactionsOp {
    ...
    /**
     * This method helps us organize a list of dates and calculations that is saved in the Account class.
     * @param month month is the main parameter that will be used to make calculations
                    ...
                    if (dateFormat.format(t.getDate()).equals("01")) {
                        ...
                        switch (t.getType()) {
                            case Deposit: {
                                ...
                                if (postion == 0) {
                                    transactionOp.setSubtotal(transactionOp.getAmount());
                                    subtotal = transactionOp.getAmount();
                                } else {
                                    transactionOp.setSubtotal((subtotal + transactionOp.getAmount()));
                                    subtotal += transactionOp.getAmount();
                                }
                    ...
                    if (dateFormat.format(t.getDate()).equals("02")) {
                        ...
                        switch (t.getType()) {
                            case Deposit: {
                                transactionOp.setDeposit(t.getAmount());
                                if (postion == 0) {
                                    transactionOp.setSubtotal((janauary + transactionOp.getAmount()));
                                    subtotal = janauary + transactionOp.getAmount();
                                } else {
                                    transactionOp.setSubtotal((int) (subtotal + transactionOp.getAmount()));
                                    subtotal += transactionOp.getAmount();
                                }
                            }
                            case Withdrawal: {
                    ...
                    if (dateFormat.format(t.getDate()).equals("03")) {
                        ...
                        switch (t.getType()) {
                            case Deposit: {
                                transactionOp.setDeposit(t.getAmount());
                                if (postion == 0) {
                                    transactionOp.setSubtotal((janauary + february + transactionOp.getAmount()));
                                    subtotal = janauary + february + transactionOp.getAmount();
                                } else {
                                    transactionOp.setSubtotal((subtotal + transactionOp.getAmount()));
                                    subtotal += transactionOp.getAmount();
                                }
                    ...
                    if (dateFormat.format(t.getDate()).equals("04")) {
                        ...
                        switch (t.getType()) {
                            case Deposit: {
                                transactionOp.setDeposit(t.getAmount());
                                if (postion == 0) {
                                    transactionOp.setSubtotal((janauary + february + march + transactionOp.getAmount()));
                                    subtotal = janauary + february + march + transactionOp.getAmount();
                                } else {
                                    transactionOp.setSubtotal((subtotal + transactionOp.getAmount()));
                                    subtotal += transactionOp.getAmount();
                                }
                    ...
                    if (dateFormat.format(t.getDate()).equals("05")) {
                        ...
                        switch (t.getType()) {
                            case Deposit: {
                                transactionOp.setDeposit(t.getAmount());
                                if (postion == 0) {
                                    transactionOp.setSubtotal((janauary + february + march + april + transactionOp.getAmount()));
                                    subtotal = janauary + february + march + april + transactionOp.getAmount();
                                } else {
                                    transactionOp.setSubtotal((int) (subtotal + transactionOp.getAmount()));
                                    subtotal += transactionOp.getAmount();
                                }
                    ...
                    if (dateFormat.format(t.getDate()).equals("06")) {
                        ...
                        switch (t.getType()) {
                            case Deposit: {
                                transactionOp.setDeposit(t.getAmount());
                                if (postion == 0) {
                                    transactionOp.setSubtotal((janauary + february + march + april + may + transactionOp.getAmount()));
                                    subtotal = janauary + february + march + april + may + transactionOp.getAmount();
                                } else {
                                    transactionOp.setSubtotal((subtotal + transactionOp.getAmount()));
                                    subtotal += transactionOp.getAmount();
                                }
                    ...
                    if (dateFormat.format(t.getDate()).equals("07")) {
                        ...
                        switch (t.getType()) {
                            case Deposit: {
                                transactionOp.setDeposit(t.getAmount());
                                if (postion == 0) {
                                    transactionOp.setSubtotal((janauary + february + march + april + may + june + transactionOp.getAmount()));
                                    subtotal = janauary + february + march + april + may + june + transactionOp.getAmount();
                                } else {
                                    transactionOp.setSubtotal((subtotal + transactionOp.getAmount()));
                                    subtotal += transactionOp.getAmount();
                                }
                    ...
                    if (dateFormat.format(t.getDate()).equals("08")) {
                        ...
                        switch (t.getType()) {
                            case Deposit: {
                                transactionOp.setDeposit(t.getAmount());
                                if (postion == 0) {
                                    transactionOp.setSubtotal((janauary + february + march + april + may + june + july + transactionOp.getAmount()));
                                    subtotal = janauary + february + march + april + may + june + july + transactionOp.getAmount();
                                } else {
                                    transactionOp.setSubtotal((subtotal + transactionOp.getAmount()));
                                    subtotal += transactionOp.getAmount();
                                }
                    ...
                    if (dateFormat.format(t.getDate()).equals("09")) {
                        ...
                        switch (t.getType()) {
                            case Deposit: {
                                transactionOp.setDeposit(t.getAmount());
                                if (postion == 0) {
                                    transactionOp.setSubtotal((janauary + february + march + april + may + june + july + august + transactionOp.getAmount()));
                                    subtotal = janauary + february + march + april + may + june + july + august + transactionOp.getAmount();
                                } else {
                                    transactionOp.setSubtotal((subtotal + transactionOp.getAmount()));
                                    subtotal += transactionOp.getAmount();
                                }
                            ...
                            case Withdrawal: {
                                transactionOp.setWithDraw(t.getAmount());
                                if (postion == 0) {
                                    transactionOp.setSubtotal((janauary + february + march + april + may + june + august - transactionOp.getAmount()));
                                    subtotal = janauary + february + march + april + may + june + august - transactionOp.getAmount();
                                } else {
                                    transactionOp.setSubtotal((subtotal - transactionOp.getAmount()));
                                    subtotal -= transactionOp.getAmount();
                    ...
                    if (dateFormat.format(t.getDate()).equals("10")) {
                        ...
                        switch (t.getType()) {
                            case Deposit: {
                                transactionOp.setDeposit(t.getAmount());
                                if (postion == 0) {
                                    transactionOp.setSubtotal((janauary + february + march + april + may + june + july + august + september + transactionOp.getAmount()));
                                    subtotal = janauary + february + march + april + may + june + july + august + september + transactionOp.getAmount();
                                } else {
                                    transactionOp.setSubtotal((subtotal + transactionOp.getAmount()));
                                    subtotal += transactionOp.getAmount();
                                }
                    ...
                    if (dateFormat.format(t.getDate()).equals("11")) {
                        ...
                        switch (t.getType()) {
                            case Deposit: {
                                transactionOp.setDeposit(t.getAmount());
                                if (postion == 0) {
                                    transactionOp.setSubtotal((janauary + february + march + april + may + june + july + august + september + october + transactionOp.getAmount()));
                                    subtotal = janauary + february + march + april + may + june + july + august + september + october + transactionOp.getAmount();
                                } else {
                                    transactionOp.setSubtotal((subtotal + transactionOp.getAmount()));
                                    subtotal += transactionOp.getAmount();
                                }
                    ...
                    if (dateFormat.format(t.getDate()).equals("12")) {
                        ...
                        switch (t.getType()) {
                            case Deposit: {
                                transactionOp.setDeposit(t.getAmount());
                                if (postion == 0) {
                                    transactionOp.setSubtotal((janauary + february + march + april + may + june + september + october + november + transactionOp.getAmount()));
                                    subtotal = janauary + february + march + april + may + june + september + october + november + transactionOp.getAmount();
                                } else {
                                    transactionOp.setSubtotal((subtotal + transactionOp.getAmount()));
                                    subtotal += transactionOp.getAmount();
                                }
